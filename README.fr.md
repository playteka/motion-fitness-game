...
```bash
npm test                       # les cinq suites d'un coup (925 tests)
...
| `tests/test-detectors.mjs` | 234 | Comptage, chronométrage, points par étape et ordre de validation des cinq détecteurs écrits à la main (mouvements corrects comme erronés), ainsi que la logique de validation du calibrage |
...
| `tests/test-app.mjs` | 208 | Démarrage du vrai `app.js`, rendu de la page d'accueil, fenêtre de réglages, déroulé du calibrage, changement d'exercice, score, sons, bilan, changement de langue |
