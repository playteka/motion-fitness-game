...
**Why it used to be unresponsive** (four causes, found by reading the code, all four fixed):

| Cause | Before | Now |
|---|---|---|
| ① **The hit area was far smaller than the ring** | the hit radius was **0.30 of the diameter** (60 % of the ring radius): the ring drawn on screen has a 70 px radius, but only a **42 px** disc in the middle counted — so aiming “inside the ring” landed in a dead zone most of the time | **0.45** (90 % of the ring radius): **anywhere inside the ring counts**, with the outer 10 % left free so merely grazing it does not fire |
| ② **Visibility gate too high, and at least 2 landmarks required** | the gate was **0.4** and a hand needed **at least 2** of wrist/index/pinky/thumb above it — but a hand or foot pushed into a **frame corner** (which is exactly where this ring lives) has low visibility by nature, so the whole hand/foot was thrown away and the ring never moved | the gate is **0.3** and **a single usable landmark is enough** (the wrist on its own is very stable) |
...
| ④ **No way to tell “not detected” from “nearly there”** | only being inside the ring produced any feedback, so a hand right next to it looked like nothing was happening | within **2.2×** the hit radius the ring **lights up** (`.near`: a cyan glow — colour only, no size change), so you can see at once that “the camera has my hand, just move it a little further in” |

- **A diagnostic line was added**: open the 🐞 panel and one of the last rows now reads
  **“Hand/foot hand(0.09,0.85) from ring centre 0.42 inside / nearly there / outside”** (or “no hand or foot seen”) —
  “is my hand even being detected, and how far off is it?” is answered right there instead of by guesswork.
...
> exactly on the detector's own progress line (2252 assertions in the suite).
...
```bash
npm test                       # all six suites (2252 assertions)
