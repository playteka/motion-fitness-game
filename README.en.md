...
- **Hands or feet**: a hand is judged by the average of the wrist + index + pinky + thumb (the palm centre) and a foot by the
  **ankle**; either one reaching the ring's centre counts (points with visibility below 0.4 are ignored, so a hand or foot hidden
  behind your body cannot trigger it).
...
> exactly on the detector's own progress line (2233 assertions in the suite).
