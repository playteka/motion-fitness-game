...
- **It is there the whole time you are on the exercise page** (calibration / 3-2-1 countdown / counting / paused), so you do not
  have to finish a set first — with sweaty hands and no keyboard nearby you can just bail out.
...
- **Exactly the same animation as the two post-set rings**: the same `ring-btn / ring-svg / ring-fill` markup, the same clockwise
  ...
  three things differ: it is pinned to the **bottom-left corner**, its diameter is one notch smaller (0.11 of the frame width, so
  it does not cover the action), and firing leaves the set **and fullscreen**.
- **Hugging the corner without covering the bar**: the judgement bar is drawn along the bottom edge of the frame (about 12% of the
  height), so the ring's lower edge stays above it (at 16:9 that edge is ≈ 0.74 + 0.098 ≈ 0.84 < 0.86) — a test pins that geometry.
