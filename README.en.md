...
**Why it used to be unresponsive** (five causes found across two rounds of feedback, all fixed):
...
- **How “any part entering is enough” works**: every hand landmark (wrist / index / pinky / thumb) and every foot landmark
  (ankle / heel / toe tip) is its own sample point, and **any one** of them landing inside the ring's circle starts the hourglass;
  points with visibility below 0.3 are ignored (so a hand or foot hidden behind your body cannot trigger it). Poking in with just a
  fingertip, or just a toe, therefore works.
...
> exactly on the detector's own progress line (2265 assertions in the suite).
...
```bash
npm test                       # all six suites (2265 assertions)
