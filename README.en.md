...
- **Hands or feet**: a hand is judged by the average of the wrist + index + pinky + thumb (the **palm centre**) and a foot by the
  ...
  either one reaching the ring's centre counts (points with visibility below 0.4 are ignored, so a hand or foot hidden
...
- **Exactly the same animation as the two post-set rings**: the same `ring-btn / ring-svg / ring-fill` markup, the same clockwise
  ...
  these differ: it is pinned to the **bottom-left corner**, hands or feet both work, and firing leaves the set **and fullscreen**.
- **Diagonal to the top-right HUD progress ring, and always the same size**: both rings take their diameter from one CSS variable,
  `--ring-px` (`clamp(96px, 10vw, 132px)`) — change that single number and both change together, so “all the rings are the same
  size” is a style-level constraint instead of two hand-maintained numbers (`tests/test-page.mjs` guards it). The horizontal insets
  are symmetric too: 18 px from the left edge here, 18 px from the right edge up top.
- **Hugging the corner without covering the bar**: the judgement bar is drawn along the bottom edge of the frame (`bottom: 12px`,
  roughly 98–110 px tall), so the ring's **lower edge stays 122 px above the bottom** (`CORNER_RING_BOTTOM_PX`) — a test pins that
  geometry. That is why this ring's **vertical** inset is larger than the top-right one's (16 px): the whole bottom strip belongs to
  the judgement bar, so the two rings are strictly symmetric horizontally and shifted apart vertically.
...
> exactly on the detector's own progress line (2226 assertions in the suite).
...
> **“Side Plank” was removed at the user's request** (Core now holds five: Plank / Dead Bug / Crunch / Reverse Crunch /
> Lying Leg Raise — it was the only side-lying exercise, and the camera can only judge “lying on your side on one forearm”
> roughly, which put it in the “can't be judged reliably” group).
...
```bash
npm test                       # all six suites (2226 assertions)
