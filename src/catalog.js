...
export const EXERCISES = [
  ...
  e('buttKick', '🏃', 'lower', {
    ...
    params: {
      ...
      onValue: 126, offValue: 132, minRepMs: 80, leadMin: 18,
      /**
       * **第二路证据**（用户反馈「跳得时候经常无法计数，尤其是跳得快的时候」后补的）：
       * 快节奏 + 动作模糊 + 平滑滤波会把**采样到的**最小膝角压浅（实测差 10~30°），
       * 而膝角那条线必须卡得很紧（松一点「原地小跑」就会被算成勾腿，见 tests 的边界用例）。
       * 所以补一路**跨度大得多**、且与膝角无关的证据：**脚跟到同侧髋的距离 ÷ 腿长**
       * （`perSide.kick`，见 metrics.js）：站立 ≈1.0、原地小跑 ≥0.95、真正勾到臀部 ≈0.45~0.7
       * （实测：折 120° 的真勾腿在 1000ms/轮到 250ms/轮之间读 0.58~0.69）。
       * 两路证据取「**或**」，谁先到线算谁的 —— 和臀桥 / 平板支撑那套「或」判据同一个思路。
       */
      altMetric: 'kick', altDip: 0.12, altKneeDip: 0.06, altLeadMin: 0.06,
  ...
  e('boxJump', '🦘', 'full', {
     ...
     *   - 画面里画一个**箱子**贴在用户正前方（跟着人走），箱顶就是判定线；
     *   - 箱高按**用户自己的站立膝高**定（0.55 × 膝高 ≈ 25cm），机位远近/个子高矮都跟得上；
     *   - **脚的最低点越过箱顶**就计一次（`BOXJUMP`，见 exercises.js），越过那一刻立刻报数；
     *   - 落回地面才允许下一次（迟滞），所以站在箱子前不会刷数。
     *
     * 以前是「整个人离地 ≥0.035×画面高」就算一次（粗略判定）—— 现在有了箱子，
     * 判据变成「跳过箱顶」，而且**画面上的箱顶和判定的那条线是同一个数**（都是 `boxLine`）。
     */
    engine: 'builtin', plan: 'boxJump', view: 'front', posture: 'stand', judge: 'box', target: 10, rough: true,
    // 姿势门控仍然是站姿（不依赖地面线的那一版，和勾腿跳同一个：只看躯干倾角 + 肩高于髋）
    params: { gate: 'standUpright' },
  }),
