...
export function computeFrame(metric, calib, now, use3d = false, world = null) {
  ...
  for (const s of ['L', 'R']) {
    ...
    perSide[s] = {
      ...
      /**
       * 「**脚跟往臀部勾起来**」的程度 = 脚跟到同侧髋的距离 ÷ 腿长（大腿 + 小腿）。
       *
       * 勾腿跳的第二个证据（用户反馈「跳得快的时候经常无法计数」）：
       *   站立 ≈ **1.0**（脚跟大致在髋下方一整条腿的位置）；真正勾到大腿后侧 ≈ **0.45~0.6**；
       *   而「原地小跑只抬膝、脚跟没往臀部勾」依旧 ≈ **0.9 以上** —— 两者分得很开。
       *
       * 为什么不能只靠膝角：快节奏 + 动作模糊 + 平滑滤波会把**采样到的**最小膝角压浅
       * （实测差 10~30°），而膝角的判定线又必须卡得很紧（松一点「原地小跑」就会被算成勾腿）。
       * 「脚跟到髋的距离」是个**大跨度**的量（1.0 → 0.5），被平滑压掉一点也照样跨过判定线，
       * 而且它不依赖地面线、不依赖画幅比例（除以腿长），机位高低都不影响。
       */
      kick: dist2(P(I.heel), P(I.hip))
        / Math.max(1e-3, dist2(P(I.hip), P(I.knee)) + dist2(P(I.knee), P(I.ankle))),
      heelVis: P(I.heel).v,
