...
console.log('\n[10] 关键帧线条图标');
{
  ...
  // 勾腿跳（用户要求）：站立 → 勾腿（≤120°）→ 换另一条腿勾（≤120°），而且图标必须是**站姿勾腿**。
  // 阈值随「快节奏计不上」的两轮修复从 100/135 调成 120/126（见 catalog.js 里的说明）
  {
    ...
    ok('勾腿跳：三格 = 站立 → 勾腿 → 换另一条腿勾（最后一格 = 计次那一刻）',
      ...
      && stK[1].value === 120 && stK[2].value === 120 && stK[2].metric === 'otherSide',
    ...
    ok('勾腿跳：后两格画的就是那个膝角（120°），而且一近一远（换另一条腿）',
      icons[1].pose.params.kickKnee === 120 && icons[2].pose.params.kickKnee === 120
    ...
    ok('勾腿跳：最后一格的补充说明带上了「回到 ≥126°/最短间隔 0.11 秒」的参数',
      stK[2].item.noteKey === 'spec.note.altSwitch'
      && stK[2].item.noteParams?.rest === 126 && stK[2].item.noteParams?.gap === 0.11,
